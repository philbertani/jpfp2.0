...
import { editStudent, selectDbStatus, resetAddStudent} from '../slices/addStudentSlice'
...
const EditStudent = (props) => {
  ...
  function initializeState() {
    ...
    for (let i = 0; i < fields.length; i++) {
      ...
      if (typeof studentData[field] === "object") {
        ...
        //weird if null goes through toString it comes back as an object
        //need to prevent displaying 'null' in GPA field
        if (value === 'null') value =''
      }
      else {  }//console.log(i,field)      }
  ...
  let fields = []
  if ( studentData !== null ) fields = Object.keys(studentData);
  let dbValues = [];
  ...
  function intercept(ev) {
    ...
    for (let i = 0; i < userFields.length; i++) {
      ...
      //allow update with null GPA
      const fieldIsJustWhiteSpaceOrNull =
        String(inp[dbIndex]).replace(/\s\s+/g, " ") === " " || inp[dbIndex] === "";
      if ( StudentsModel.isNum[i] && fieldIsJustWhiteSpaceOrNull) {
        studentForm[name] = null;
      } else if ( StudentsModel.isNum[i]) {
        const isValidNum = parseFloat(inp[dbIndex]);
      }
    ...
    dispatch(
      editStudent({ studentForm, campusId, studentId: Number(studentId) })
    );
  }
  ...
  if (Object.keys(editStudentResult).length>0 && editStudentResult.hasOwnProperty('id')) {
    console.log('ids',studentId, editStudentResult.id)
    if ( studentId != editStudentResult.id) {
    ...
    return [
      ...
      >
        ...
        <div id="campusSelect">
          ...
          <select
            onChange={(ev) => {
              setCampusId(ev.target.value);
            }}
          >
            {campusOptions}
          </select>
        </div>
        <input className="genericSubmit" type="submit" value="UPDATE"></input>
      </form>,
      <div className="infoTabs2">
        <div style={{ marginRight: "1vw", marginLeft: "1vw" }}>
          ...
          <p style={{ wordBreak: "break-all" }}>
            {JSON.stringify(editStudentResult)}
          </p>
          <h3>
            <Link className="addStudentEditLink" to={"/students/" + studentId}>
              Home Page
            </Link>
          </h3>
        </div>
